...
<title>recv函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">recv（经socket接收数据）<br>
	相关函数 recvfrom，recvmsg，send，sendto，socket<br>
	表头文件 #include&lt;sys/types.h&gt;<br>
	#include&lt;sys/socket.h&gt;<br>
	定义函数 int recv(int s,void *buf,int len,unsigned int flags);<br>
	函数说明 recv()用来接收远端主机经指定的socket传来的数据，并把数据存到由参数buf 指向的内存空间，参数len为可接收数据的最大长度。<br>
	参数 flags一般设0。其他数值定义如下:<br>
	MSG_OOB 接收以out-of-band 送出的数据。<br>
	MSG_PEEK 返回来的数据并不会在系统内删除，如果再调用recv()会<br>
	返回相同的数据内容。<br>
	MSG_WAITALL强迫接收到len大小的数据后才能返回，除非有错误或<br>
	信号产生。<br>
	MSG_NOSIGNAL此操作不愿被SIGPIPE信号中断返回值成功则返回接收<br>
	到的字符数，失败返回-1，错误原因存于errno中。<br>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="recv.php#" class="bds_more" data-cmd="more"></a>
	<a href="recv.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="recv.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="recv.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="recv.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="recv.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
