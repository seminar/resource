...
<title>lfind函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">lfind（线性搜索）<br>
	相关函数 lsearch<br>
	表头文件 #include&lt;stdlib.h&gt;<br>
	定义函数 void *lfind (const void *key,const void *base,size_t *nmemb,size_t<br>
	size,int(* compar) (const void * ,const void *));<br>
	函数说明 lfind()利用线性搜索在数组中从头至尾一项项查找数据。参数key指向欲查找的关键数据，参数base指向要被搜索的数组开头地址，参数nmemb代表数组中的元素数量，每一元素的大小则由参数size决定，最后一项参数compar为一函数指针，这个函数用来判断两个元素是否相同，若传给compar 
	的异地个参数所指的元素数据和第二个参数所指的元素数据相同时则返回0，两个元素数据不相同则返回非0值。Lfind()与lsearch()不同点在于，当找不到关键数据时lfind()仅会返回NULL，而不会主动把该笔数据加入数组尾端。<br>
	返回值 找到关键数据则返回找到的该笔元素的地址，如果在数组中找不到关键数据则返回空指针(NULL)。<br>
　</td>
	</tr>
</table>
　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="lfind.php#" class="bds_more" data-cmd="more"></a>
	<a href="lfind.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="lfind.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="lfind.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="lfind.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="lfind.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
