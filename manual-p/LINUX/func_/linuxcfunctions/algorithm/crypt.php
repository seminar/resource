...
<title>crypt函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">crypt（将密码或数据编码）<br>
	相关函数 getpass<br>
	表头文件 #define _XOPEN_SOURCE<br>
	#include&lt;unistd.h&gt;<br>
	定义函数 char * crypt (const char *key,const char * salt);<br>
	函数说明 crypt()将使用Data Encryption Standard(DES)演算法将参数key所指的字符串加以编码，key字符串长度仅取前8个字符，超过此长度的字符没有意义。参数salt为两个字符组成的字符串，由a-z、AZ、0-9，“.”和“/”所组成，用来决定使用4096 
	种不同内建表格的哪一个。函数执行成功后会返回指向编码过的字符串指针，参数key 所指的字符串不会有所更动。编码过的字符串长度为13 
	个字符，前两个字符为参数salt代表的字符串。<br>
	返回值 返回一个指向以NULL结尾的密码字符串。<br>
	附加说明 使用GCC编译时需加-lcrypt。<br>
	　</td>
	</tr>
</table>
　</td>
...
<tr><td align="right" width="398"> 
	...
	执行 Input First Password: /* 输入test，编码后存于passwd[ ] */<br>
	Input Second Password /*输入test，密码相同编码后也会相同*/<br>
	After crypt () 1st Passwd : teH0wLIpW0gyQ<br>
	After crypt () 2nd Passwd : teH0wLIpW0gyQ</td>
	</tr>
</table>
　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="crypt.php#" class="bds_more" data-cmd="more"></a>
	<a href="crypt.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="crypt.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="crypt.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="crypt.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="crypt.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
