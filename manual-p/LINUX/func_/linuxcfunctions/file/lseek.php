...
<title>lseek函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="403"> 
	<p align="left">lseek（移动文件的读写位置）<br>
	相关函数 dup，open，fseek<br>
	表头文件 #include&lt;sys/types.h&gt;<br>
	#include&lt;unistd.h&gt;<br>
	定义函数 off_t lseek(int fildes,off_t offset ,int whence);<br>
	函数说明 每一个已打开的文件都有一个读写位置，当打开文件时通常其读写<br>
	位置是指向文件开头，若是以附加的方式打开文件(如O_APPEND)，<br>
	则读写位置会指向文件尾。当read()或write()时，读写位置会随之<br>
	增加，lseek()便是用来控制该文件的读写位置。参数fildes 为已<br>
	打开的文件描述词，参数offset 为根据参数whence来移动读写位置<br>
	的位移数。<br>
	参数 whence为下列其中一种:<br>
	SEEK_SET 参数offset即为新的读写位置。<br>
	SEEK_CUR 以目前的读写位置往后增加offset个位移量。<br>
	SEEK_END 将读写位置指向文件尾后再增加offset个位移量。<br>
	当whence 值为SEEK_CUR 或SEEK_END时，参数offet允许负值的出<br>
	现。<br>
	下列是教特别的使用方式:<br>
	1) 欲将读写位置移到文件开头时:lseek（int<br>
	fildes,0,SEEK_SET）；<br>
	2) 欲将读写位置移到文件尾时:lseek（int fildes，<br>
	0,SEEK_END）；<br>
	3) 想要取得目前文件位置时:lseek（int fildes，0,SEEK_CUR）；<br>
	返回值 当调用成功时则返回目前的读写位置，也就是距离文件开头多少个<br>
	字节。若有错误则返回-1，errno 会存放错误代码。<br>
	附加说明 Linux系统不允许lseek（）对tty装置作用，此项动作会令lseek<br>
	（）返回ESPIPE。<br>
	　</td>
	</tr>
</table>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="lseek.php#" class="bds_more" data-cmd="more"></a>
	<a href="lseek.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="lseek.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="lseek.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="lseek.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="lseek.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
