...
<title>read函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">read（由已打开的文件读取数据）<br>
	相关函数 readdir，write，fcntl，close，lseek，readlink，fread<br>
	表头文件 #include&lt;unistd.h&gt;<br>
	定义函数 ssize_t read(int fd,void * buf ,size_t count);<br>
	函数说明 read()会把参数fd 所指的文件传送count个字节到buf指针所指的内<br>
	存中。若参数count为0，则read()不会有作用并返回0。返回值为实<br>
	际读取到的字节数，如果返回0，表示已到达文件尾或是无可读取的<br>
	数据，此外文件读写位置会随读取到的字节移动。<br>
	附加说明 如果顺利read()会返回实际读到的字节数，最好能将返回值与参数<br>
	count 作比较，若返回的字节数比要求读取的字节数少，则有可能<br>
	读到了文件尾、从管道(pipe)或终端机读取，或者是read()被信号<br>
	中断了读取动作。当有错误发生时则返回-1，错误代码存入errno<br>
	中，而文件读写位置则无法预期。<br>
	错误代码 EINTR 此调用被信号所中断。<br>
	EAGAIN 当使用不可阻断I/O 时（O_NONBLOCK），若无数据可读取则<br>
	返回此值。<br>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="read.php#" class="bds_more" data-cmd="more"></a>
	<a href="read.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="read.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="read.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="read.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="read.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
