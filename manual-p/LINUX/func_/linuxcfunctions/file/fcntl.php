...
<title>fcntl函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<li><a href="creat.php">creat</a></li>
<li><a href="dup.php">dup</a></li>
<li><a href="dup2.php">adup2</a></li>
<li><a href="fcntl.php">fcntl</a></li>
<li><a href="flock.php">flock</a></li>
<li><a href="fsync.php">fsync</a></li>
<li><a href="lseek.php">lseek</a></li>
<li><a href="mkstemp.php">mkstemp</a></li>
...
<tr><td align="right" width="398"> 
	<p align="left">fcntl（文件描述词操作）<br>
	相关函数 open，flock<br>
	表头文件 #include&lt;unistd.h&gt;<br>
	#include&lt;fcntl.h&gt;<br>
	定义函数 int fcntl(int fd , int cmd);<br>
	int fcntl(int fd,int cmd,long arg);<br>
	int fcntl(int fd,int cmd,struct flock * lock);<br>
	函数说明 fcntl()用来操作文件描述词的一些特性。参数fd代表欲设置的文件<br>
	描述词，参数cmd代表欲操作的指令。<br>
	有以下几种情况:<br>
	F_DUPFD用来查找大于或等于参数arg的最小且仍未使用的文件描述<br>
	词，并且复制参数fd的文件描述词。执行成功则返回新复制的文件<br>
	描述词。请参考dup2()。F_GETFD取得close-on-exec旗标。若此旗<br>
	标的FD_CLOEXEC位为0，代表在调用exec()相关函数时文件将不会关<br>
	闭。<br>
	F_SETFD 设置close-on-exec 旗标。该旗标以参数arg 的<br>
	FD_CLOEXEC位决定。<br>
	F_GETFL 取得文件描述词状态旗标，此旗标为open（）的参数<br>
	flags。<br>
	F_SETFL 设置文件描述词状态旗标，参数arg为新旗标，但只允许<br>
	O_APPEND、O_NONBLOCK和O_ASYNC位的改变，其他位的改变将不受影<br>
	响。<br>
	F_GETLK 取得文件锁定的状态。<br>
	F_SETLK 设置文件锁定的状态。此时flcok 结构的l_type 值必须是<br>
	F_RDLCK、F_WRLCK或F_UNLCK。如果无法建立锁定，则返回-1，错误<br>
	代码为EACCES 或EAGAIN。<br>
	F_SETLKW F_SETLK 作用相同，但是无法建立锁定时，此调用会一直<br>
	等到锁定动作成功为止。若在等待锁定的过程中被信号中断时，会<br>
	立即返回-1，错误代码为EINTR。参数lock指针为flock 结构指针，<br>
	定义如下<br>
	struct flcok<br>
	{<br>
	...
	返回值 成功则返回0，若有错误则返回-1，错误原因存于errno.</td>
	</tr>
</table>
　</td>
    <td background="../img_css/frame11.gif" width="14">　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="fcntl.php#" class="bds_more" data-cmd="more"></a>
	<a href="fcntl.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="fcntl.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="fcntl.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="fcntl.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="fcntl.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
