...
<title>flock函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">flock（锁定文件或解除锁定）<br>
	相关函数 open,fcntl<br>
	表头文件 #include&lt;sys/file.h&gt;<br>
	定义函数 int flock(int fd,int operation);<br>
	函数说明 flock()会依参数operation所指定的方式对参数fd所指的文件做各<br>
	种锁定或解除锁定的动作。此函数只能锁定整个文件，无法锁定文<br>
	件的某一区域。<br>
	参数 operation有下列四种情况:<br>
	LOCK_SH 建立共享锁定。多个进程可同时对同一个文件作共享锁<br>
	定。<br>
	LOCK_EX 建立互斥锁定。一个文件同时只有一个互斥锁定。<br>
	LOCK_UN 解除文件锁定状态。<br>
	LOCK_NB 无法建立锁定时，此操作可不被阻断，马上返回进程。通<br>
	常与LOCK_SH或LOCK_EX 做OR(|)组合。<br>
	单一文件无法同时建立共享锁定和互斥锁定，而当使用dup()或fork<br>
	()时文件描述词不会继承此种锁定。<br>
	返回值 返回0表示成功，若有错误则返回-1，错误代码存于errno。</td>
	</tr>
</table>
　</td>
    <td background="../img_css/frame11.gif" width="14">　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="flock.php#" class="bds_more" data-cmd="more"></a>
	<a href="flock.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="flock.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="flock.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="flock.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="flock.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
