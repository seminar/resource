...
<title>isatty函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<li><a href="../env/setenv.php">setenv</a></li>
  ...
  <h3>终端控制函数 </h3>
  <ul>
<li><a href="getopt.php">getopt</a></li>
<li><a href="isatty.php">isatty</a></li>
<li><a href="ttyname.php">ttyname</a></li>
</ul>
  <h3>进程控制函数 </h3>
  <ul>
...
<tr><td align="right" width="398"> 
	<p align="left">isatty（判断文件描述词是否是为终端机）<br>
	相关函数 ttyname<br>
	表头文件 #include&lt;unistd.h&gt;<br>
	定义函数 int isatty(int desc);<br>
	函数说明 如果参数desc所代表的文件描述词为一终端机则返回1，否则返回<br>
	0。<br>
	返回值 如果文件为终端机则返回1，否则返回0。<br>
	select（I/O多工机制）<br>
	表头文件 #include&lt;sys/time.h&gt;<br>
	#include&lt;sys/types.h&gt;<br>
	#include&lt;unistd.h&gt;<br>
	定义函数 int select(int n,fd_set * readfds,fd_set * writefds,fd_set<br>
	* exceptfds,struct timeval * timeout);<br>
	函数说明 select()用来等待文件描述词状态的改变。参数n代表最大的文件描<br>
	述词加1，参数readfds、writefds 和exceptfds 称为描述词组，是<br>
	用来回传该描述词的读，写或例外的状况。底下的宏提供了处理这<br>
	三种描述词组的方式:<br>
	FD_CLR(inr fd,fd_set* set)；用来清除描述词组set中相关fd 的<br>
	位<br>
	FD_ISSET(int fd,fd_set *set)；用来测试描述词组set中相关fd<br>
	的位是否为真<br>
	FD_SET（int fd,fd_set*set）；用来设置描述词组set中相关fd的<br>
	位<br>
	FD_ZERO（fd_set *set）； 用来清除描述词组set的全部位<br>
	参数 timeout为结构timeval，用来设置select()的等待时间，其结构定<br>
	义如下<br>
	struct timeval<br>
	{<br>
	time_t tv_sec;<br>
	...
	返回值 如果参数timeout设为NULL则表示select（）没有timeout。<br>
	错误代码 执行成功则返回文件描述词状态已改变的个数，如果返回0代表在描<br>
	述词状态改变前已超过timeout时间，当有错误发生时则返回-1，错<br>
	误原因存于errno，此时参数readfds，writefds，exceptfds和<br>
	timeout的值变成不可预测。<br>
	EBADF 文件描述词为无效的或该文件已关闭<br>
	EINTR 此调用被信号所中断<br>
	EINVAL 参数n 为负值。<br>
	ENOMEM 核心内存不足<br>
	范例 常见的程序片段:fs_set readset；<br>
	FD_ZERO(&amp;readset);<br>
	FD_SET(fd,&amp;readset);<br>
	select(fd+1,&amp;readset,NULL,NULL,NULL);<br>
	if(FD_ISSET(fd,readset){……}</td>
	</tr>
</table>
　</td>
    <td background="../img_css/frame11.gif" width="14">　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="isatty.php#" class="bds_more" data-cmd="more"></a>
	<a href="isatty.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="isatty.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="isatty.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="isatty.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="isatty.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
