...
<title>sacnf函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="411"> 
	<p align="left">sacnf（格式化字符串输入）<br>
	相关函数 fscanf，snprintf<br>
	表头文件 #include&lt;stdio.h&gt;<br>
	定义函数 int scanf(const char * format,.......);<br>
	函数说明<br>
	scanf()会将输入的数据根据参数format字符串来转换并格式化数<br>
	据。Scanf()格式转换的一般形式如下<br>
	％[*][size][l][h]type<br>
	以中括号括起来的参数为选择性参数，而％与type则是必要的。<br>
	* 代表该对应的参数数据忽略不保存。<br>
	size 为允许参数输入的数据长度。<br>
	l 输入的数据数值以long int 或double型保存。<br>
	h 输入的数据数值以short int 型保存。<br>
	底下介绍type的几种形式<br>
	％d 输入的数据会被转成一有符号的十进制数字（int）。<br>
	％i 输入的数据会被转成一有符号的十进制数字，若输入数据以<br>
	“0x”或“0X”开头代表转换十六进制数字，若以“0”开头则转换<br>
	八进制数字，其他情况代表十进制。<br>
	％0 输入的数据会被转换成一无符号的八进制数字。<br>
	％u 输入的数据会被转换成一无符号的正整数。<br>
	％x 输入的数据为无符号的十六进制数字，转换后存于unsigned<br>
	int型变量。<br>
	％X 同％x<br>
	％f 输入的数据为有符号的浮点型数，转换后存于float型变量。<br>
	％e 同％f<br>
	％E 同％f<br>
	％g 同％f<br>
	％s 输入数据为以空格字符为终止的字符串。<br>
	％c 输入数据为单一字符。<br>
	[] 读取数据但只允许括号内的字符。如[a-z]。<br>
	[^] 读取数据但不允许中括号的^符号后的字符出现，如[^0-9].<br>
	返回值 成功则返回参数数目，失败则返回-1，错误原因存于errno中。<br>
　</td>
	</tr>
</table>
　</td>
...
<tr><td align="right" width="398"> 
	...
	执行 10 0x1b aaaaaaaaaa bbbbbbbbbb<br>
	10 27 aaaaa</td>
	</tr>
</table>
　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="sacnf.php#" class="bds_more" data-cmd="more"></a>
	<a href="sacnf.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="sacnf.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="sacnf.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="sacnf.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="sacnf.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
