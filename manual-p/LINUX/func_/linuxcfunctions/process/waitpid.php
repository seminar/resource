...
<title>waitpid函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<img border="0" src="../img_css/line.JPG" width="406" height="7"></div>
  ...
  <tr> 
    <td background="../img_css/frame10.gif" width="12">　</td>
    <td>
<table width="402" border="0" align="center" class="space" id="table35">
<tr><td align="right" width="396"> 
	<p align="left">waitpid（等待子进程中断或结束）<br>
	相关函数 wait，fork<br>
	表头文件 #include&lt;sys/types.h&gt;<br>
	#include&lt;sys/wait.h&gt;<br>
	定义函数 pid_t waitpid(pid_t pid,int * status,int options);<br>
	函数说明 waitpid()会暂时停止目前进程的执行，直到有信号来到或子进程结束。如果在调用wait()时子进程已经结束，则wait()会立即返回子进程结束状态值。子进程的结束状态值会由参数status返回，而子进程的进程识别码也会一快返回。如果不在意结束状态值，则参数status可以设成NULL。参数pid为欲等待的子进程识别码，其他数值意义如下:<br>
	...
	WUNTRACED 如果子进程进入暂停执行情况则马上返回，但结束状态<br>
	不予以理会。<br>
	子进程的结束状态返回后存于status，底下有几个宏可判别结束情况<br>
	WIFEXITED(status)如果子进程正常结束则为非0值。<br>
	WEXITSTATUS(status)取得子进程exit()返回的结束代码，一般会先<br>
	用WIFEXITED 来判断是否正常结束才能使用此宏。<br>
	WIFSIGNALED(status)如果子进程是因为信号而结束则此宏值为真<br>
	WTERMSIG(status)取得子进程因信号而中止的信号代码，一般会先<br>
	用WIFSIGNALED 来判断后才使用此宏。<br>
	WIFSTOPPED(status)如果子进程处于暂停执行情况则此宏值为真。<br>
	一般只有使用WUNTRACED 时才会有此情况。<br>
	WSTOPSIG(status)取得引发子进程暂停的信号代码，一般会先用<br>
	WIFSTOPPED 来判断后才使用此宏。<br>
	返回值 如果执行成功则返回子进程识别码(PID)，如果有错误发生则返回-1。失败原因存于errno中。<br>
	　</td>
	</tr>
</table>
　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="waitpid.php#" class="bds_more" data-cmd="more"></a>
	<a href="waitpid.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="waitpid.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="waitpid.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="waitpid.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="waitpid.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
