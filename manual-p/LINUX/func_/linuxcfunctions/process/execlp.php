...
<title>execlp函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">execlp（从PATH 环境变量中查找文件并执行）<br>
	相关函数 fork，execl，execle，execv，execve，execvp<br>
	表头文件 #include&lt;unistd.h&gt;<br>
	定义函数 int execlp(const char * file,const char * arg,……)；<br>
	函数说明 execlp()会从PATH 
	环境变量所指的目录中查找符合参数file的文件名，找到后便执行该文件，然后将第二个以后的参数当做该文件的argv[0]、argv[1]……，最后一个参数必须用空指针(NULL)作结束。<br>
	返回值 如果执行成功则函数不会返回，执行失败则直接返回-1，失败原因存于errno 中。<br>
	错误代码 参考execve()。<br>
　</td>
	</tr>
</table>
...
<tr><td align="right" width="398"> 
	<p align="left">范例:</p>
	<p align="left">/* 执行ls -al /etc/passwd execlp()会依PATH 变量中的/bin找到/bin/ls */<br>
	#include&lt;unistd.h&gt;<br>
	main()<br>
	{<br>
	execlp(“ls”,”ls”,”-al”,”/etc/passwd”,(char *)0);<br>
	}<br>
	执行 -rw-r--r-- 1 root root 705 Sep 3 13 :52 /etc/passwd</td>
	</tr>
</table>
　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="execlp.php#" class="bds_more" data-cmd="more"></a>
	<a href="execlp.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="execlp.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="execlp.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="execlp.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="execlp.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
