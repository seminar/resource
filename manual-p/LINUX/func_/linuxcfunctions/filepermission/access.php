...
<title>access函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">access（判断是否具有存取文件的权限）<br>
	相关函数 stat，open，chmod，chown，setuid，setgid<br>
	表头文件 #include&lt;unistd.h&gt;<br>
	定义函数 int access(const char * pathname,int mode);<br>
	函数说明 access()会检查是否可以读/写某一已存在的文件。参数mode有几种情况组合，R_OK，W_OK，X_OK 和F_OK。R_OK，W_OK与X_OK用来检查文件是否具有读取、写入和执行的权限。F_OK则是用来判断该文件是否存在。由于access()只作权限的核查，并不理会文件形态或文件内容，因此，如果一目录表示为“可写入”，表示可以在该目录中建立新文件等操作，而非意味此目录可以被当做文件处理。例如，你会发现DOS的文件都具有“可执行”权限，但用execve()执行时则会失败。<br>
	返回值 若所有欲查核的权限都通过了检查则返回0值，表示成功，只要有一权限被禁止则返回-1。<br>
	错误代码 EACCESS 参数pathname 所指定的文件不符合所要求测试的权限。<br>
	...
	EIO I/O 存取错误。<br>
	附加说明 使用access()作用户认证方面的判断要特别小心，例如在access()后再做open()的空文件可能会造成系统安全上的问题。</td>
	</tr>
</table>
　</td>
    <td background="../img_css/frame11.gif" width="14">　</td>
...
<tr><td align="right" width="398"> 
	<p align="left">范例:</p>
	<p align="left">/* 判断是否允许读取/etc/passwd */<br>
	#include&lt;unistd.h&gt;<br>
	int main()<br>
	{<br>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="access.php#" class="bds_more" data-cmd="more"></a>
	<a href="access.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="access.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="access.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="access.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="access.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
