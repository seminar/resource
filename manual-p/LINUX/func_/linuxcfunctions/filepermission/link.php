...
<title>link函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">link（建立文件连接）<br>
	相关函数 symlink，unlink<br>
	表头文件 #include&lt;unistd.h&gt;<br>
	定义函数 int link (const char * oldpath,const char * newpath);<br>
	函数说明 link()以参数newpath指定的名称来建立一个新的连接(硬连接)到参数oldpath所指定的已存在文件。如果参数newpath指定的名称为一已存在的文件则不会建立连接。<br>
	返回值 成功则返回0，失败返回-1，错误原因存于errno。<br>
	附加说明 link()所建立的硬连接无法跨越不同文件系统，如果需要请改用symlink()。<br>
	错误代码<br>
	EXDEV 参数oldpath与newpath不是建立在同一文件系统。<br>
	EPERM 参数oldpath与newpath所指的文件系统不支持硬连接<br>
	EROFS 文件存在于只读文件系统内<br>
	EFAULT 参数oldpath或newpath 指针超出可存取内存空间。<br>
	ENAMETOLLONG 参数oldpath或newpath太长<br>
	ENOMEM 核心内存不足<br>
	EEXIST 参数newpath所指的文件名已存在。<br>
	EMLINK 参数oldpath所指的文件已达最大连接数目。<br>
...
<tr><td align="right" width="398"> 
	<p align="left">范例:</p>
	<p align="left">/* 建立/etc/passwd 的硬连接为pass */<br>
	#include&lt;unistd.h&gt;<br>
	main()<br>
	{<br>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="link.php#" class="bds_more" data-cmd="more"></a>
	<a href="link.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="link.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="link.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="link.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="link.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
