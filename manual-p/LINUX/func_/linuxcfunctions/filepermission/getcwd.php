...
<title>getcwd函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">getcwd（取得当前的工作目录）<br>
	相关函数 get_current_dir_name，getwd，chdir<br>
	表头文件 #include&lt;unistd.h&gt;<br>
	定义函数 char * getcwd(char * buf,size_t size);<br>
	函数说明 getcwd()会将当前的工作目录绝对路径复制到参数buf所指的内存空间，参数size为buf的空间大小。在调用此函数时，buf所指的内存空间要足够大，若工作目录绝对路径的字符串长度超过参数size大小，则回值NULL，errno的值则为ERANGE。倘若参数buf为NULL，getcwd()会依参数size的大小自动配置内存(使用malloc())，如果参数size也为0，则getcwd()会依工作目录绝对路径的字符串程度来决定所配置的内存大小，进程可以在使用完此字符串后利用free()来释放此空间。<br>
	返回值 执行成功则将结果复制到参数buf所指的内存空间，或是返回自动配置的字符串指针。失败返回NULL，错误代码存于errno。<br>
　</td>
	</tr>
</table>
　</td>
...
<tr><td align="right" width="398"> 
	<p align="left">范例:</p>
	<p align="left">范例 #include&lt;unistd.h&gt;<br>
	main()<br>
	{<br>
	char buf[80];<br>
	...
	执行 current working directory :/tmp</td>
	</tr>
</table>
　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="getcwd.php#" class="bds_more" data-cmd="more"></a>
	<a href="getcwd.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="getcwd.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="getcwd.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="getcwd.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="getcwd.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
