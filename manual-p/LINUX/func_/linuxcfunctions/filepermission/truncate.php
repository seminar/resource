...
<title>truncate函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">truncate（改变文件大小）<br>
	相关函数 open，ftruncate<br>
	表头文件 #include&lt;unistd.h&gt;<br>
	定义函数 int truncate(const char * path,off_t length);<br>
	函数说明 truncate()会将参数path 指定的文件大小改为参数length 
	指定的大小。如果原来的文件大小比参数length大，则超过的部分会被删去。<br>
	返回值 执行成功则返回0，失败返回-1，错误原因存于errno。<br>
	错误代码 EACCESS 参数path所指定的文件无法存取。<br>
	EROFS 欲写入的文件存在于只读文件系统内<br>
	EFAULT 参数path指针超出可存取内存空间<br>
	EINVAL 参数path包含不合法字符<br>
	ENAMETOOLONG 参数path太长<br>
	ENOTDIR 参数path路径并非一目录<br>
	EISDIR 参数path 指向一目录<br>
	ETXTBUSY 参数path所指的文件为共享程序，而且正被执行中<br>
	ELOOP 参数path’有过多符号连接问题<br>
	EIO I/O 存取错误。</td>
	</tr>
</table>
　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="truncate.php#" class="bds_more" data-cmd="more"></a>
	<a href="truncate.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="truncate.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="truncate.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="truncate.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="truncate.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
