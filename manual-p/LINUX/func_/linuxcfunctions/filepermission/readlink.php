...
<title>readlink函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">readlink（取得符号连接所指的文件）<br>
	相关函数 stat，lstat，symlink<br>
	表头文件 #include&lt;unistd.h&gt;<br>
	定义函数 int readlink(const char * path ,char * buf,size_t bufsiz);<br>
	函数说明 readlink()会将参数path的符号连接内容存到参数buf所指的内存空间，返回的内容不是以NULL作字符串结尾，但会将字符串的字符数返回。若参数bufsiz小于符号连接的内容长度，过长的内容会被截断。<br>
	返回值 执行成功则传符号连接所指的文件路径字符串，失败则返回-1，错误代码存于errno。<br>
	错误代码 EACCESS 取文件时被拒绝，权限不够<br>
	EINVAL 参数bufsiz 为负数<br>
	EIO I/O 存取错误。<br>
	ELOOP 欲打开的文件有过多符号连接问题。<br>
	ENAMETOOLONG 参数path的路径名称太长<br>
	ENOENT 参数path所指定的文件不存在<br>
	ENOMEM 核心内存不足<br>
	ENOTDIR 参数path路径中的目录存在但却非真正的目录。</td>
	</tr>
</table>
　</td>
    <td background="../img_css/frame11.gif" width="14">　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="readlink.php#" class="bds_more" data-cmd="more"></a>
	<a href="readlink.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="readlink.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="readlink.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="readlink.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="readlink.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
