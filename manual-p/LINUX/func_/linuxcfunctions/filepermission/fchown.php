...
<title>fchown函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">fchown（改变文件的所有者）<br>
	相关函数 chown，lchown，chmod<br>
	表头文件 #include&lt;sys/types.h&gt;<br>
	#include&lt;unistd.h&gt;<br>
	定义函数 int fchown(int fd,uid_t owner,gid_t group);<br>
	函数说明 fchown()会将参数fd指定文件的所有者变更为参数owner代表的用<br>
	户，而将该文件的组变更为参数group组。如果参数owner或group<br>
	为-1，对映的所有者或组有所改变。参数fd 为已打开的文件描述<br>
	词。当root用fchown()改变文件所有者或组时，该文件若具S_ISUID<br>
	或S_ISGID权限，则会清除此权限位。<br>
	返回值 成功则返回0，失败则返回-1，错误原因存于errno。<br>
	错误代码 EBADF 参数fd文件描述词为无效的或该文件已关闭。<br>
	EPERM 进程的有效用户识别码与欲修改权限的文件所有者不同，而<br>
	且也不具root权限，或是参数owner、group不正确。<br>
	EROFS 欲写入的文件存在于只读文件系统内。<br>
	ENOENT 指定的文件不存在<br>
	EIO I/O存取错误<br>
　</td>
	</tr>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="fchown.php#" class="bds_more" data-cmd="more"></a>
	<a href="fchown.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="fchown.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="fchown.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="fchown.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="fchown.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
