...
<title>getpwent函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">getpwent（从密码文件中取得账号的数据）<br>
	相关函数 getpw，fgetpwent，getpwnam，getpwuid，setpwent，endpwent<br>
	表头文件 #include&lt;pwd.h&gt;<br>
	#include&lt;sys/types.h&gt;<br>
	定义函数 strcut passwd * getpwent(void);<br>
	函数说明 getpwent（）用来从密码文件（/etc/passwd）中读取一项用户数<br>
	据，该用户的数据以passwd 结构返回。第一次调用时会取得第一位<br>
	用户数据，之后每调用一次就会返回下一项数据，直到已无任何数<br>
	据时返回NULL。<br>
	passwd 结构定义如下<br>
	struct passwd{<br>
	...
	返回值 返回passwd 结构数据，如果返回NULL 则表示已无数据，或有错误<br>
	发生。<br>
	附加说明 getpwent()在第一次调用时会打开密码文件，读取数据完毕后可使<br>
	用endpwent()来关闭该密码文件。错误代码ENOMEM 内存不足，无法<br>
	配置passwd结构。<br>
　</td>
	</tr>
</table>
...
<tr><td align="right" width="398"> 
	<p align="left">范例:</p>
	<p align="left">#include&lt;pwd.h&gt;<br>
	#include&lt;sys/types.h&gt;<br>
	main()<br>
	{<br>
	struct passwd *user;<br>
	while((user = getpwent())!=0){<br>
	printf(“%s:%d:%d:%s:%s:%s\n”,user-&gt;pw_name,user-<br>
	&gt;pw_uid,user-&gt;pw_gid,<br>
	user-&gt;pw_gecos,user-&gt;pw_dir,user-&gt;pw_shell);<br>
	}<br>
	endpwent();<br>
	}<br>
	执行 root:0:0:root:/root:/bin/bash<br>
	bin:1:1:bin:/bin:<br>
	...
	kids:500:500: : /home/kids:/bin/bash</td>
	</tr>
</table>
　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="getpwent.php#" class="bds_more" data-cmd="more"></a>
	<a href="getpwent.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="getpwent.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="getpwent.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="getpwent.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="getpwent.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
