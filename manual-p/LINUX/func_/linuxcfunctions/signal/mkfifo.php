...
<title>mkfifo函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">mkfifo（建立具名管道）<br>
	相关函数 pipe，popen，open，umask<br>
	表头文件 #include&lt;sys/types.h&gt;<br>
	#include&lt;sys/stat.h&gt;<br>
	定义函数 int mkfifo(const char * pathname,mode_t mode);<br>
	函数说明 mkfifo()会依参数pathname建立特殊的FIFO文件，该文件必须不存<br>
	在，而参数mode为该文件的权限（mode%~umask），因此umask值也<br>
	会影响到FIFO文件的权限。Mkfifo()建立的FIFO文件其他进程都可<br>
	以用读写一般文件的方式存取。当使用open()来打开FIFO文件时，<br>
	O_NONBLOCK旗标会有影响<br>
	1、当使用O_NONBLOCK 旗标时，打开FIFO 文件来读取的操作会立刻<br>
	返回，但是若还没有其他进程打开FIFO 文件来读取，则写入的操作<br>
	会返回ENXIO 错误代码。<br>
	2、没有使用O_NONBLOCK 旗标时，打开FIFO 来读取的操作会等到其<br>
	他进程打开FIFO文件来写入才正常返回。同样地，打开FIFO文件来<br>
	写入的操作会等到其他进程打开FIFO 文件来读取后才正常返回。<br>
	返回值 若成功则返回0，否则返回-1，错误原因存于errno中。<br>
	错误代码 EACCESS 参数pathname所指定的目录路径无可执行的权限<br>
	...
	EROFS 参数pathname指定的文件存在于只读文件系统内。<br>
　</td>
	</tr>
</table>
　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="mkfifo.php#" class="bds_more" data-cmd="more"></a>
	<a href="mkfifo.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="mkfifo.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="mkfifo.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="mkfifo.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="mkfifo.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
