...
<title>sigaction函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<tr><td align="right" width="398"> 
	<p align="left">sigaction（查询或设置信号处理方式）<br>
	相关函数 signal，sigprocmask，sigpending，sigsuspend<br>
	表头文件 #include&lt;signal.h&gt;<br>
	定义函数 int sigaction(int signum,const struct sigaction<br>
	*act ,struct sigaction *oldact);<br>
	函数说明 sigaction()会依参数signum指定的信号编号来设置该信号的处理函数。参数signum可以指定SIGKILL和SIGSTOP以外的所有信号。<br>
	...
	sa_mask 用来设置在处理该信号时暂时将sa_mask 指定的信号搁<br>
	置。<br>
	sa_restorer 此参数没有使用。<br>
	sa_flags 用来设置信号处理的其他相关操作，下列的数值可用。<br>
	OR 运算（|）组合A_NOCLDSTOP : 如果参数signum为SIGCHLD，则当子进程暂停时并不会通知父进程SA_ONESHOT/SA_RESETHAND:当调用新的信号处理函数前，将此信号处理方式改为系统预设的方式。<br>
	SA_RESTART:被信号中断的系统调用会自行重启<br>
	SA_NOMASK/SA_NODEFER:在处理此信号未结束前不理会此信号的再次<br>
	到来。如果参数oldact不是NULL指针，则原来的信号处理方式会由此结构sigaction 返回。<br>
	返回值 执行成功则返回0，如果有错误则返回-1。<br>
	错误代码 EINVAL 参数signum 不合法， 或是企图拦截<br>
	SIGKILL/SIGSTOPSIGKILL信号<br>
...
<tr><td align="right" width="398"> 
	...
	执行 sa_handler of signal 5 = Default action<br>
	...
	sa_handler of signal 14 = Default action</td>
	</tr>
</table>
　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="sigaction.php#" class="bds_more" data-cmd="more"></a>
	<a href="sigaction.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="sigaction.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="sigaction.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="sigaction.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="sigaction.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
