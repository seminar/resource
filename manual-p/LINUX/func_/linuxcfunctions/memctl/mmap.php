...
<title>mmap函数_Linux C 中文函数手册_Linux编程_linux_操作系统__www.iteedu.com</title>
...
<li><a href="../signal/strerror.php">strerror</a></li>
  ...
  <h3>内存控制函数 </h3>
  <ul>
<li><a href="calloc.php">calloc</a></li>
<li><a href="free.php">free</a></li>
<li><a href="getpagesize.php">getpagesize</a></li>
<li><a href="malloc.php">malloc</a></li>
<li><a href="mmap.php">mmap</a></li>
<li><a href="munmap.php">munmap</a></li>
</ul>
<h3>字符测试函数 </h3>
  <ul>
...
<tr><td align="right" width="398"> 
	<p align="left">mmap（建立内存映射）<br>
	相关函数 munmap，open<br>
	表头文件 #include &lt;unistd.h&gt;<br>
	#include &lt;sys/mman.h&gt;<br>
	定义函数 void *mmap(void *start,size_t length,int prot,int flags,int<br>
	fd,off_t offsize);<br>
	函数说明 mmap()用来将某个文件内容映射到内存中，对该内存区域的存取即是直接对该文件内容的读写。参数start指向欲对应的内存起始地址，通常设为NULL，代表让系统自动选定地址，对应成功后该地址会返回。参数length代表将文件中多大的部分对应到内存。<br>
	参数 prot代表映射区域的保护方式有下列组合<br>
	PROT_EXEC 映射区域可被执行<br>
	PROT_READ 映射区域可被读取<br>
	PROT_WRITE 映射区域可被写入<br>
	PROT_NONE 映射区域不能存取<br>
	参数 flags会影响映射区域的各种特性<br>
	MAP_FIXED 如果参数start所指的地址无法成功建立映射时，则放弃<br>
	映射，不对地址做修正。通常不鼓励用此旗标。<br>
	MAP_SHARED对映射区域的写入数据会复制回文件内，而且允许其他<br>
	映射该文件的进程共享。<br>
	MAP_PRIVATE 对映射区域的写入操作会产生一个映射文件的复制，<br>
	即私人的“写入时复制”（copy on write）对此区域作的任何修改<br>
	都不会写回原来的文件内容。<br>
	MAP_ANONYMOUS建立匿名映射。此时会忽略参数fd，不涉及文件，而<br>
	且映射区域无法和其他进程共享。<br>
	MAP_DENYWRITE只允许对映射区域的写入操作，其他对文件直接写入<br>
	的操作将会被拒绝。<br>
	MAP_LOCKED 将映射区域锁定住，这表示该区域不会被置换<br>
	（swap）。<br>
	在调用mmap()时必须要指定MAP_SHARED 或MAP_PRIVATE。参数fd为<br>
	open()返回的文件描述词，代表欲映射到内存的文件。参数offset<br>
	为文件映射的偏移量，通常设置为0，代表从文件最前方开始对应，<br>
	offset必须是分页大小的整数倍。<br>
	返回值 若映射成功则返回映射区的内存起始地址，否则返回MAP_FAILED(－1)，错误原因存于errno 中。<br>
	错误代码 EBADF 参数fd 不是有效的文件描述词<br>
	EACCES 存取权限有误。如果是MAP_PRIVATE 情况下文件必须可读，<br>
	使用MAP_SHARED则要有PROT_WRITE以及该文件要能写入。<br>
...
<tr><td align="right" width="398"> 
	<p align="left">范例:</p>
	<p align="left">/* 利用mmap()来读取/etc/passwd 文件内容*/<br>
	...
	void *start;<br>
	struct stat sb;<br>
	fd=open(“/etc/passwd”,O_RDONLY); /*打开/etc/passwd*/<br>
	...
	执行 root : x : 0 : root : /root : /bin/bash<br>
	...
	kids : x : 500 :500 :/home/kids : /bin/bash</td>
	</tr>
</table>
　</td>
...
﻿<div style="float:right;margin-right:100px;" class="bdsharebuttonbox">
	<a href="mmap.php#" class="bds_more" data-cmd="more"></a>
	<a href="mmap.php#" class="bds_qzone" data-cmd="qzone" title="分享到QQ空间"></a>
	<a href="mmap.php#" class="bds_tsina" data-cmd="tsina" title="分享到新浪微博"></a>
	<a href="mmap.php#" class="bds_tqq" data-cmd="tqq" title="分享到腾讯微博"></a>
	<a href="mmap.php#" class="bds_renren" data-cmd="renren" title="分享到人人网"></a>
	<a href="mmap.php#" class="bds_weixin" data-cmd="weixin" title="分享到微信"></a>
</div>
<div style="clear:both;"></div>
</div></div>
<!--end of file-wrpper-->
